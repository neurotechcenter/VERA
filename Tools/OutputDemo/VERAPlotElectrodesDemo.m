...
[remap,cmap,names,name_id] = createColormapFromAnnotations(brainmat.surfaceModel);

% Plot brain model using VERA function (uses trisurf)
figure;
plot3DModel(gca,brainmat.surfaceModel.Model,remap);
...
plotBallsOnVolume(gca,brainmat.electrodes.Location,[],elec_radius);
...
alpha(0.3);
for i = 1:size(brainmat.electrodes.Definition,1)
    plot3([brainmat.electrodes.Location(elecCtr,1), brainmat.electrodes.Location(elecCtr + NumElecs(i)-1,1)],...
        ...
        [brainmat.electrodes.Location(elecCtr,3), brainmat.electrodes.Location(elecCtr + NumElecs(i)-1,3)],'LineWidth',6)
...
cell_array_labels = cellfun(@(x)x{1},brainmat.electrodes.Label,'UniformOutput',false); % convert to easier to use cell array
...
for i=1:length(unique_labels)
    ...
    plotBallsOnVolume(gca,brainmat.electrodes.Location(elecsToPlot,:),label_cols(i,:),elec_radius);
end
view(-114,25)
