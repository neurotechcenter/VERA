function obj_write_manual_mtl(FV, filename, Ka, varargin)
...
% version please see SYS_obj_write_manual_mtl.m
%
% Syntax:  
...
% Created: 06-Jun-2020 ; Last revision: 10-Jun-2020 
...
parse(p, FV, filename, Ka, varargin{:});
...
if ~isfile(obj_filename) % new file if no previous file with the same name exists in the current folder
    ...
    [~, Mcount]=system(['grep -c ''newmtl '' ', strcat(filename, '.mtl')]);
...
switch n
    case 1  % same material is assigned for all faces (all input variable havee exactly one value)
        ...
        % % write corresponding .mtl file
            ...
            fprintf(fileID,'Ka %.4f %.4f %.4f \n', Ka);
            fprintf(fileID,'Kd %.4f %.4f %.4f \n', Kd);
            fprintf(fileID,'Ks %.4f %.4f %.4f \n', Ks);
            fprintf(fileID,'Tf %.4f %.4f %.4f \n', Tf);
