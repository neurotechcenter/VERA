...
%}
    ...
    try
        ...
        % Disabled for VERA: getJavaImage() below calls jPanel.paint(...) synchronously
        % from the MATLAB interpreter thread instead of the AWT Event Dispatch Thread.
        % Against VERA's GUI-Layout-Toolbox figures this reliably deadlocks on the very
        % first export (figure renders fine, export_fig then hangs forever). Always fall
        % through to the print()-based path in the catch block below instead.
        if false
            [A, alpha] = getJavaImage(fig);
        else
            error('getJavaImage disabled for VERA - see comment above; using print() fallback');
        end
    catch err  %#ok<NASGU>
