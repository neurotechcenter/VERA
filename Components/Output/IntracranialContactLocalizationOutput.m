classdef IntracranialContactLocalizationOutput < AComponent
    %IntracranialContactLocalizationOutput Creates a .mat file as Output of 
    %VERA with information about electrode locations, the surface model, 
    %and any surface annotations and electrode labels generated. It also
    %stores the 
    properties
        ...
        EEGNamesIdentifier
        ICLocalizationResultsPathIdentifier
    end

    methods
        function obj = IntracranialContactLocalizationOutput()
        ...
        function Publish(obj)
            ...
            obj.AddInput(obj.ICLocalizationResultsPathIdentifier, 'PathInformation');
        ...
        function [] = Process(obj, eLocs, eDef, surf, results_path, varargin)
            ...
            % IntracranialContactLocalization results
            % ProjectName
            projectpath     = fileparts(obj.ComponentPath);
            [~,ProjectName] = fileparts(projectpath);

            load(fullfile(results_path.Path,'imaging_processing_temp_folder',[ProjectName,'_contact_data_table_struct.mat']));
            load(fullfile(results_path.Path,'imaging_processing_temp_folder',[ProjectName,'_imaging_processing_info_struct_update.mat']));

            shank_model_all = imaging_processing_info_struct.shank_model_all;

            save(fullfile(path,file),'surfaceModel','electrodes','electrodeNamesKey','contact_data_struct','shank_model_all');
