function createIndivYeoMapping(fsDir, segPath)
...
labelFolder=fullfile(segPath,'label');

avg_dir=fullfile(fsDir,'subjects','fsaverage');
...
y17labels = {'no_label_found';'VisCent';'VisPeri';'SomMotA';'SomMotB';'DAN-A';'DAN-B';'Sal-VAN-A';
                                 'SAL-VAN-B';'Limbic-B';'Limbic-A';'FPCN-C';'FPCN-A';'FPCN-B';'DMN-D';'DMN-C';'DMN-A';'DMN-B'};
...
for hemLoop=1:2
    ...
    %% Read Sub Pial Surf
    fname=[segPath filesep 'surf' filesep hem '.pial'];
    pial=read_surf_helper(fname);
    
    
    %% Read Sub Spherical Surf
    fname=[segPath filesep 'surf' filesep hem '.sphere.reg'];
    ...
    [avgBrainYeo7, label7, colortable7]=read_annotation(fullfile(fsDir,'subjects','fsaverage','label',fname7));
    [avgBrainYeo17, label17, colortable17]=read_annotation(fullfile(fsDir,'subjects','fsaverage','label',fname17));
    ...
    for b=2:18
        colortable17.struct_names{b}=y17labels{b};
    end
